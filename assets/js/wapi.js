...
window.addEventListener("message", function (event) {
  if (
    ...
    if (event.data.cmd == "findUserId") {
      ...
      if(event.data.id.chat_Id)
        {
          chatId=event.data.id.chat_Id;
        }
        else 
        {
      ...
      // WAPI.sendMessageToID(chatId, event.data.text);
    } else if (event.data.cmd == "openChat") {
      const chat = window.Store.Chat.get(event.data.id);
      if (chat != undefined) {
        window.Store.Cmd.default._openChat(chat);
      } else {
        console.log("chat open called in wapi");
        const phone = event.data.id.substring(0, event.data.id.length - 5);
      ...
      // console.log(window.Store.Chat._models[0]);
      // var firstChat = window.Store.Chat._models[0];
      // var originalID = firstChat.id;
      // firstChat.id = new window.Store.UserConstructor(event.data.id, {
      //   intentionallyUsePrivateConstructor: true,
      // });
      // console.log(firstChat.id);
      // firstChat.id = originalID;
      // const chatListLength = window.Store.Chat._models.length;
      // console.log(chatListLength);
      // const chat = window.Store.Chat._models[chatListLength];

      // window.Store.Cmd.default.openChatFromUnread(chat);
      ...
      if (contactName === undefined) {
        console.log("1", contactName);
        contactName = contact.__x_notifyName;
        console.log("2", contactName);
      }
    ...
    } else if (event.data.cmd == "getContactId") {
      console.log("initiating id search");

      window.Store.Chat._models.forEach((element) => {
        if (element.__x_active) {
          console.log("found active popup");
          window.postMessage(
            {
              id: element.__x_id._serialized,
              res: "contactId",
              direction: "from-page-script",
            },
            "*"
...
if (!window.Store) {
  (function () {
    function getStore(modules) {
      ...
      let neededObjects = [
        { id: "Cmd", conditions: (module) => (module.Cmd ? module : null) },
...
window.WAPI.sendMessage = function (chatid, message, done) {
  var id = new window.Store.UserConstructor(chatid, {
    intentionallyUsePrivateConstructor: true,
  });
